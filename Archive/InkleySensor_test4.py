...
CAN_CHANNEL = 'COM5'  # CANABLE Default
    ...
    def stream_data(self):
        ...
        try:
            if not self.initialize_can_bus():
                ...
                print("\n[DEBUG] Entered stream_data() thread", flush=True)
                print("[DEBUG] Dumping raw CAN frames for 5 seconds...", flush=True)
                ...
                while time.time() - t0 < 5:
                    ...
                    if msg is None:
                        continue
                    count += 1
                    print(f"[DEBUG] ID=0x{msg.arbitration_id:X} ext={msg.is_extended_id} dlc={msg.dlc} data={msg.data.hex()}", flush=True)

                print(f"[DEBUG] Done. Frames seen: {count}\n", flush=True)
    ...
    def do_start(self, arg):
        ...
        if not self.streaming:
            if self.send_command(self.CMD_START_STREAM):
                print("[DEBUG] Starting stream thread now...", flush=True)
                self.streaming = True
                self.stream_thread = threading.Thread(target=self.stream_data)
                self.stream_thread.start()
                print("Started real-time streaming", flush=True)
            else:
